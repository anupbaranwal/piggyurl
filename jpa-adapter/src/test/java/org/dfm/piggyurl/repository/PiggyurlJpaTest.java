...
public class PiggyurlJpaTest {
  ...
  public void shouldGiveThePiggyurlWhenAskedByIdGivenThatPiggyurlByThatIdExistsInDatabase() {
    // Given from @Sql
    // When
    Optional<Piggyurl> piggyurl = obtainPiggyurl.getPiggyurlByCode(1L);
    // Then
    assertThat(piggyurl).isNotNull().isNotEmpty().get().isEqualTo(Piggyurl.builder().code(1L).description("Twinkle twinkle little star").build());
  }
  ...
  public void shouldGiveNoPiggyurlWhenAskedByIdGivenThatPiggyurlByThatIdDoesNotExistInDatabase() {
    // Given from @Sql
    // When
    Optional<Piggyurl> piggyurl = obtainPiggyurl.getPiggyurlByCode(-1000L);
    // Then
